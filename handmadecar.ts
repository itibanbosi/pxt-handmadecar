...
namespace eureka_blocks_car {
    ...
    */
        ...
        switch (sinkou_houkou) {
            case direction.前:
                if (con_le >= 0) {
                    ...
                    pins.servoWritePin(AnalogPin.P0, 90 + (90 * (con_op + 100)) / 100);
                }
                if (con_le < 0) {
                    pins.servoWritePin(AnalogPin.P1, 90 - (90 * (con_op + 100)) / 100);
                    pins.servoWritePin(AnalogPin.P0, 90 + (90 * (con_op + 100)) / 100 + con_le);
                }
                break;
            case direction.後:
                if (con_le >= 0) {
    ...
    export function pro_LR(lot_houkou: lotation, set_LR: number): void {
        switch (lot_houkou) {
            case lotation.左:
                pins.servoWritePin(AnalogPin.P1, 90 + (90 * set_LR) / 100);
